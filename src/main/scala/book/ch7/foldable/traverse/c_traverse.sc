...
object c_traverse {
  ...
  val hostnames = List(
    "alpha.example.com",
    "beta.example.com",
    "gamma.demo.com")                             //> hostnames  : List[String] = List(alpha.example.com, beta.example.com, gamma.
                                                  //| demo.com)

  def getUptime(hostname: String): Future[Int] =
    Future(hostname.length * 60) // just for demonstration
                                                  //> getUptime: (hostname: String)scala.concurrent.Future[Int]
  ...
  val allUptimes: Future[List[Int]] =
    ...
    }                                             //> allUptimes  : scala.concurrent.Future[List[Int]] = Future(<not completed>)
                                                  //| 
  Await.result(allUptimes, 1.second)              //> res0: List[Int] = List(1020, 960, 840)
  ...
  val allUptimes2: Future[List[Int]] =
    Future.traverse(hostnames)(getUptime)         //> allUptimes2  : scala.concurrent.Future[List[Int]] = Future(Success(List(102
                                                  //| 0, 960, 840)))
  Await.result(allUptimes2, 1.second)             //> res1: List[Int] = List(1020, 960, 840)
...
*/
  ...
  List.empty[Int].pure[Future]                    //> res2: scala.concurrent.Future[List[Int]] = Future(Success(List()))
...
*/
  ...
  def newCombine(
    ...
    (accum, getUptime(host)).mapN(_ :+ _)         //> newCombine: (accum: scala.concurrent.Future[List[Int]], host: String)scala.
                                                  //| concurrent.Future[List[Int]]
  /*
...
*/
  ...
  val totalUptime = listTraverse(hostnames)(getUptime)
                                                  //> totalUptime  : scala.concurrent.Future[List[Int]] = Future(<not completed>)
                                                  //| 

  Await.result(totalUptime, 1.second)             //> res3: List[Int] = List(1020, 960, 840)
  ...
  listSequence(List(Vector(1, 2), Vector(3, 4)))  //> res4: scala.collection.immutable.Vector[List[Int]] = Vector(List(1, 3), Lis
                                                  //| t(1, 4), List(2, 3), List(2, 4))

  /*
		What about a list of three parameters?
	*/
  listSequence(List(Vector(1, 2), Vector(3, 4), Vector(5, 6)))
                                                  //> res5: scala.collection.immutable.Vector[List[Int]] = Vector(List(1, 3, 5), 
                                                  //| List(1, 3, 6), List(1, 4, 5), List(1, 4, 6), List(2, 3, 5), List(2, 3, 6), 
  ...
  def process(inputs: List[Int]) =
    listTraverse(inputs)(n => if (n % 2 == 0) Some(n) else None)
                                                  //> process: (inputs: List[Int])Option[List[Int]]
  ...
  process(List(2, 4, 6))                          //> res6: Option[List[Int]] = Some(List(2, 4, 6))
  process(List(1, 2, 3))                          //> res7: Option[List[Int]] = None
  ...
  val numbers = List(Future(1), Future(2), Future(3))
                                                  //> numbers  : List[scala.concurrent.Future[Int]] = List(Future(Success(1)), Fu
                                                  //| ture(Success(2)), Future(Success(3)))
  val numbers2: Future[List[Int]] =
    Traverse[List].sequence(numbers)              //> numbers2  : scala.concurrent.Future[List[Int]] = Future(<not completed>)
  Await.result(numbers2, 1.second)                //> res11: List[Int] = List(1, 2, 3)
  ...
  Await.result(hostnames.traverse(getUptime), 1.second)
                                                  //> res12: List[Int] = List(1020, 960, 840)

  Await.result(numbers.sequence, 1.second)        //> res13: List[Int] = List(1, 2, 3)
