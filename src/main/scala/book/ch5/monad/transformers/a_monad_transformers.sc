  ...
  println("Welcome to the Scala worksheet")       //> Welcome to the Scala worksheet
	...
	*/
		...
		val result1: ListOption[Int] = OptionT(List(Option(10)))
                                                  //> result1  : book.ch5.monad.transformers.a_monad_transformers.ListOption[Int]
                                                  //|  = OptionT(List(Some(10)))

		val result2: ListOption[Int] = 32.pure[ListOption]
                                                  //> result2  : book.ch5.monad.transformers.a_monad_transformers.ListOption[Int]
                                                  //|  = OptionT(List(Some(32)))
	...
	*/
		...
		val a = 10.pure[ErrorOrOption]    //> a  : book.ch5.monad.transformers.a_monad_transformers.ErrorOrOption[Int] = 
                                                  //| OptionT(Right(Some(10)))

		val b = 32.pure[ErrorOrOption]    //> b  : book.ch5.monad.transformers.a_monad_transformers.ErrorOrOption[Int] = 
                                                  //| OptionT(Right(Some(32)))

		val c = a.flatMap(x => b.map(y => x + y))
                                                  //> c  : cats.data.OptionT[book.ch5.monad.transformers.a_monad_transformers.Err
                                                  //| orOr,Int] = OptionT(Right(Some(42)))
	...
	*/
		...
		val futureEitherOr: FutureEitherOption[Int] =
			...
			} yield a1 + b1           //> futureEitherOr  : book.ch5.monad.transformers.a_monad_transformers.FutureEi
                                                  //| therOption[Int] = OptionT(EitherT(Future(<not completed>)))
	...
	*/
			...
			123.pure[EitherT[Option, String, ?]]
                                                  //> res1: cats.data.EitherT[Option,String,Int] = EitherT(Some(Right(123)))
	...
	*/
		...
		val errorStack1 = OptionT[ErrorOr, Int](Right(Some(10)))
                                                  //> errorStack1  : cats.data.OptionT[book.ch5.monad.transformers.a_monad_trans
                                                  //| formers.ErrorOr,Int] = OptionT(Right(Some(10)))

		// Create using pure:
		val errorStack2 = 32.pure[ErrorOrOption]
                                                  //> errorStack2  : book.ch5.monad.transformers.a_monad_transformers.ErrorOrOpt
                                                  //| ion[Int] = OptionT(Right(Some(32)))
	...
	*/
		...
		errorStack1.value                 //> res2: book.ch5.monad.transformers.a_monad_transformers.ErrorOr[Option[Int]
                                                  //| ] = Right(Some(10))

		// Mapping over the Either in the stack:
		errorStack2.value.map(_.getOrElse(-1))
                                                  //> res3: scala.util.Either[String,Int] = Right(32)
	...
	*/
		...
		futureEitherOr                    //> res4: book.ch5.monad.transformers.a_monad_transformers.FutureEitherOption[
                                                  //| Int] = OptionT(EitherT(Future(Success(Right(Some(42))))))

		val intermediate = futureEitherOr.value
                                                  //> intermediate  : book.ch5.monad.transformers.a_monad_transformers.FutureEit
                                                  //| her[Option[Int]] = EitherT(Future(Success(Right(Some(42)))))

		val stack = intermediate.value    //> stack  : scala.concurrent.Future[Either[String,Option[Int]]] = Future(Succ
                                                  //| ess(Right(Some(42))))

		Await.result(stack, 1.second)     //> res5: Either[String,Option[Int]] = Right(Some(42))
	...
	*/
		...
		def parseNumber(str: String): Logged[Option[Int]] =
			...
			}                         //> parseNumber: (str: String)book.ch5.monad.transformers.a_monad_transformers
                                                  //| .Logged[Option[Int]]
		...
		def addAll(a: String, b: String, c: String): Logged[Option[Int]] = {
			...
			result.value
		}                                 //> addAll: (a: String, b: String, c: String)book.ch5.monad.transformers.a_mon
                                                  //| ad_transformers.Logged[Option[Int]]

		// This approach doesn't force OptionT on other users' code:
		val result3 = addAll("1", "2", "3")
                                                  //> result3  : book.ch5.monad.transformers.a_monad_transformers.Logged[Option[
                                                  //| Int]] = WriterT((List(Read 1, Read 2, Read 3),Some(6)))

		val result4 = addAll("1", "a", "3")
                                                  //> result4  : book.ch5.monad.transformers.a_monad_transformers.Logged[Option[
                                                  //| Int]] = WriterT((List(Read 1, Failed on a),None))
	...
	*/
					val powerLevels = Map(
						...
						"Bumblebee" -> 8,
						"Hot Rod" -> 10
					)         //> powerLevels  : scala.collection.immutable.Map[String,Int] = Map(Jazz -> 6,
                                                  //|  Bumblebee -> 8, Hot Rod -> 10)
	/*
...
//		type Response[A] = EitherT[Future, String, A]
		...
		}                                 //> getPowerLevel: (ally: String)book.ch5.monad.transformers.a_monad_transform
                                                  //| ers.Response[Int]
			...
			def canSpecialMove(ally1: String, ally2: String): Response[Boolean] =
				for {
					...
					power2 <- getPowerLevel(ally2)
                                                  ...
                                                  //> canSpecialMove: (ally1: String, ally2: String)book.ch5.monad.transformers.
                                                  //| a_monad_transformers.Response[Boolean]
	...
	*/
		...
		}                                 //> tacticalReport: (ally1: String, ally2: String)String
	...
	*/
			tacticalReport("Jazz", "Bumblebee")
                                                  //> res6: String = Jazz and Bumblebee need a recharge.

			tacticalReport("Bumblebee", "Hot Rod")
                                                  //> res7: String = Bumblebee and Hot Rod are ready to roll out!

			tacticalReport("Jazz", "Ironhide")
                                                  //> res8: String = Comms error: Ironhide unreachable
